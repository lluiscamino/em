...
namespace em::ast::exprs {
  FunctionCall::FunctionCall(std::unique_ptr<Expression> expression, std::vector<std::unique_ptr<Expression>> arguments)
  ...
  const std::vector<std::unique_ptr<Expression>>& FunctionCall::arguments() const {
    return mArguments;
  }
