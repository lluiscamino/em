...
namespace em::utils {
void AstTreeDiagramGenerator::generate(
    const std::unique_ptr<ast::Program>& program, const std::string& filePath) {
  program->accept(*this);
...
ast::NodeVisitor::VisitorRetValue AstTreeDiagramGenerator::visit(
...
unsigned int AstTreeDiagramGenerator::createTokenNode(const Token& token) {
  return createNode(TokenTypeToString(token.type()) + ": " +
                    string::wStringToString(token.text()));
}

void AstTreeDiagramGenerator::linkNodes(
    const std::vector<unsigned int>& nodeIds, const std::string& label) {
