...
ProgramFunction::ProgramFunction(
    Token identifier, std::vector<Token> parameters,
    std::shared_ptr<ast::exprs::Expression> expression)
    : mIdentifier(std::move(identifier)),
      mParameters(std::move(parameters)),
      mExpression(std::move(expression)) {}

bool ProgramFunction::operator==(const Value& other) { return this == &other; }
...
std::string ProgramFunction::str() const {
  return "ProgramFunction<" +
         utils::string::wStringToString(mIdentifier.text()) + ">";
}

ast::NodeVisitor::VisitorRetValue ProgramFunction::execute(
    runtime::Interpreter& interpreter,
    const std::vector<std::shared_ptr<Value>>& arguments) {
  for (size_t i = 0; i < std::min(arguments.size(), mParameters.size());
       i++) {  // TODO: Create variables environment
