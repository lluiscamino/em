...
namespace em::values::functions {
  ...
  std::unique_ptr<Value> Function::isSubsetOf(const std::shared_ptr<Value>& other) const {
    throw std::logic_error("Function::isSubsetOf: not supported");
  }

  std::unique_ptr<Value> Function::hasElement(const std::shared_ptr<Value>& other) const {
  ...
  std::unique_ptr<Value> Function::unionOp(const std::shared_ptr<Value>& other) const {
    throw std::logic_error("Function::unionOp: not supported");
  }

  std::unique_ptr<Value> Function::intersection(const std::shared_ptr<Value>& other) const {
